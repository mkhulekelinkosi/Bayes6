...
All the columns are numeric excerpt for the Group column. There are 45 missing values are observed in total. The missingness patterns given the visualizations, one can conclude that the middingness is MAR because the missing values in lecturer E are missing when lecturer F And G are missing missingness is dependent on the two variables also lecturer D only one is not dependent on G and F. This could be because the the groups have already been assessed 
...
save.image("https://github.com/mkhulekelinkosi/Bayes6.git
")
```
